...
class TestEndToEndFlow:
    ...
    @pytest.mark.e2e
    @pytest.mark.asyncio
    async def test_complete_upload_flow(self, authenticated_client: AsyncClient, sample_pdf_bytes: bytes, upload_dir: Path):
        """
        ...
        health_resp = await authenticated_client.get("/api/v1/health")
        ...
        detail_resp = await authenticated_client.get(f"/api/v1/documents/{doc_ids[0]}")
        ...
        batch_resp = await authenticated_client.get(f"/api/v1/processing/batches/{batch_id}")
        ...
        audit_resp = await authenticated_client.get("/api/v1/audit/logs", params={"correlation_id": correlation_id})
        ...
        cand_resp = await authenticated_client.get("/api/v1/candidates/BGV-E2E-001")
    ...
    async def test_upload_for_existing_candidate_reuses_record(
        ...
        list_resp = await authenticated_client.get("/api/v1/candidates")
        data = list_resp.json()
    ...
    async def test_multiple_batches_for_same_candidate(
        ...
        # List batches
        batches_resp = await authenticated_client.get("/api/v1/processing/batches")
    ...
    async def test_documents_filterable_by_status(
        ...
        resp = await authenticated_client.get("/api/v1/documents", params={"status_filter": "uploaded"})
        ...
        resp_completed = await authenticated_client.get("/api/v1/documents", params={"status_filter": "completed"})
