...
_DOC_TYPE_EVIDENCE = {
    ...
    "marksheet_10th": [re.compile(r'(?i)(secondary\s+school|ssc\b|class\s*(?:x\b|10\b)|10th\b|tenth)')],
    "marksheet_12th": [re.compile(r'(?i)(senior\s+school|higher\s+secondary|hsc\b|class\s*(?:xii\b|12\b)|12th\b|twelfth|intermediate|h\.?s\.?c\b)')],
    "certificate_degree": [re.compile(r'(?i)(degree|bachelor|master|university|awarded|conferred|engineering|b\.tech|b\.sc|m\.tech|m\.sc|b\.a\b|m\.a\b)')],
    "certificate_diploma": [re.compile(r'(?i)(diploma|polytechnic|awarded|conferred)')],
    "marksheet_diploma": [re.compile(r'(?i)(diploma|polytechnic)')],
    "marksheet_degree": [re.compile(r'(?i)(bachelor|master|degree|b\.?tech|b\.?e\b|b\.?sc|b\.?com|b\.?a\b|m\.?tech|m\.?sc|m\.?b\.?a|semester|grade\s*card|credits)')],
    "college_id_card": [re.compile(r'(?i)(college|student|university|id\s*card|identity\s*card|roll\s*no|register\s*no|enrolment|wpu|mit|coep)')],
    ...
    "bank_statement": [re.compile(r'(?i)(bank|statement|balance|credit|debit|transaction|account\s+(?:no|num)|ifsc|micr|passbook|pass\s*book|branch)')],
    ...
    "fitment_medical_certificate": [re.compile(r'(?i)(medical|fitness|fitment|examined|health|doctor|clinical|dr\b)')],
}
