...
export default function ClassificationViewer({ classifications }: ClassificationViewerProps) {
  if (classifications.length === 0) {
    return (
      <div className="card">
        <h3 className="text-lg font-bold text-gray-900 mb-2">AI Classification</h3>
  ...
  return (
    <div className="card">
      <h3 className="text-lg font-bold text-gray-900 mb-4">AI Classification</h3>
          ...
          <div key={cls.id} className="border border-gray-100 rounded-xl p-4">
            <div className="flex justify-between items-center mb-3">
              <div>
                ...
                <span className="ml-3 px-2.5 py-1 bg-primary-50 text-primary-700 text-xs font-semibold rounded-lg ring-1 ring-inset ring-primary-600/10">
            ...
            {(cls.extracted_name || cls.extracted_dob || cls.extracted_id_number) && (
              <div className="bg-gray-50/80 rounded-xl p-3">
                ...
                <dl className="grid grid-cols-2 gap-2">
                  {cls.extracted_name && (
                    <div>
                      <dt className="text-[11px] font-bold uppercase tracking-widest text-gray-400">Name</dt>
                      <dd className="text-sm text-gray-900 mt-0.5">{cls.extracted_name}</dd>
                    </div>
                  )}
                  {cls.extracted_dob && (
                    <div>
                      <dt className="text-[11px] font-bold uppercase tracking-widest text-gray-400">Date of Birth</dt>
                      <dd className="text-sm text-gray-900 mt-0.5">{cls.extracted_dob}</dd>
                    </div>
                  )}
                  {cls.extracted_id_number && (
                    <div>
                      <dt className="text-[11px] font-bold uppercase tracking-widest text-gray-400">ID Number</dt>
                      <dd className="text-sm text-gray-900 mt-0.5">{cls.extracted_id_number}</dd>
                    </div>
                  )}
                </dl>
              </div>
